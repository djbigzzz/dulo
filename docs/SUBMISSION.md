...
Built solo. Next.js 15, TypeScript and Postgres, no Anchor program, 1,164 tests, CI on every push.
...
- [ ] CI is green on the default branch. `npx vitest run` printed **1,164** on 16 Sep; re-read it on the day and make the Full Description and the README say the same number.
