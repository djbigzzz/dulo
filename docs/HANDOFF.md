...
| 1:05 | Token-2022 multipliers | `src/lib/adapters/solana.ts`: `getTokenBalances` over both token programs (`getParsedTokenAccountsByOwner`), the batched `getMultipleParsedAccounts` mint read, `parseScaledUiAmountExtension`, `effectiveMultiplier`; then `normaliseQtyString` in `src/lib/assets/normalise.ts`; then `tests/solana-adapter.test.ts`; then (15 s) the corporate-actions feed: `src/lib/corporate-actions.ts` `listCorporateActions` → `GET /api/v1/partners/prestocks` `corporateActions` in the terminal (SpaceX 1 → 5, kind split; OpenAI 1 → 1.4861347, kind adjustment) → the PreStocks Partner page in the browser → `src/lib/plays/catalogue.ts` the `held_through_split` row | "xStocks are Token-2022 mints with the ScaledUiAmount extension: a split or a dividend changes a multiplier on the mint, not your raw balance. The adapter batches the mint reads, switches to the pending newMultiplier once its activation time has passed, and normalises the quantity with string decimal maths, so a holding reads the way a statement would. The same mint state feeds the corporate-actions feed: one batched read across every registered issuer, cached ten minutes, and it never throws, so a Partner page never stalls on an RPC outage. Two PreStocks adjustments are on record, SpaceX five-for-one and OpenAI times one point four eight six, and the copy says what an adjustment is: more tokens shown, the same value held. The Held Through a Split quest reads that state from the wallet's own snapshots, and both recorded adjustments predate our first snapshot, so it completes on the next one, honestly." |
| 1:35 | A quest is a data row | `src/lib/plays/catalogue.ts`, the Diversified rule `{ "type": "diversified", "minAssets": 3, "minSectors": 2 }` and one in-platform rule with `distinctBy` → `src/lib/plays/rules.ts` `PLAY_RULE_TYPES` (eight types, zod-validated) → `src/lib/plays/engine.ts` `evaluatePlay` → `evalDiversified`; then the browser, Proof on a completed on-chain quest; then (20 s) the pre-IPO trade path: `src/lib/plays/catalogue.ts` the `first_preipo_trade` row (`assetSymbols` narrows counted paper trades to the eight pre-IPO symbols) → `src/lib/games/league.ts` `LEAGUE_ASSET_SOURCES` and `placeTrade`'s `getPriceBySymbol(symbol, { sources })` → the browser: `$APP/prestocks`, one paper trade in a pre-IPO token with virtual cash, the toast "Quest complete: First Pre-IPO Trade" | "A quest is data. That rule is stored as JSON on a row and validated by a zod schema with eight rule types. In-platform quests use the same engine, counting paper trades and predictions instead of snapshots. The engine is pure functions over history: complete or not, plus the proof. This is the same JSON the player opens in the proof sheet, so adding a quest is a database row, not code. The competition takes the same route: one allowlist of asset sources, so a paper trade in a pre-IPO token quotes through the same price layer as an xStock, and the trade route completes the pre-IPO quest in the same request. Bots stay on xStocks." |
| 2:10 | A second issuer through the same interfaces | `src/lib/assets/registry.ts` `ASSET_SOURCES` (two entries: xstocks, prestocks) → `src/lib/assets/prestocks.ts` `PRESTOCKS_STATIC` (eight mints, `pythFeedId: null`) → `prisma/schema.prisma` `Play.assetSource` → `src/lib/plays/engine.ts` `scopeOf` and `inScope` → `src/lib/plays/catalogue.ts` the `pre_ipo_position` row (`assetSource: "prestocks"`) → the terminal: `npx vitest run tests/second-issuer-regression.test.ts tests/pre-ipo-quest.test.ts`, run live | "Here is the second issuer. PreStocks pre-IPO tokens are eight more Token-2022 mints, and they enter through the AssetSource registry: the snapshot reads the union of every registered mint set in the same batched RPC calls it already made, so the second issuer costs zero extra RPC calls; the resolver tags each holding with the source that claimed it; and the price layer only sends a symbol to Pyth when its source has Pyth underlyings, so a pre-IPO token is Jupiter-priced and never a Hermes call. Then the fence. Every Play row carries an assetSource, every holding carries its source, and the engine applies both: the issuer fence, then the rule's own asset list, which narrows it rather than replacing it, because two issuers can ship the same symbol. Pre-IPO Position is one more JSON row, fenced to prestocks, count-based because the price cache is empty on a cold start. The regression file runs every xStocks quest with and without a bag of pre-IPO tokens and proves the answer never moves, and the inverse: the PreStocks quest never completes on an xStocks-only wallet. One scoring path, two issuers, keyed by Play.assetSource." |
| 2:50 | The five-minute tick | `.github/workflows/tick.yml` (`*/5`) and `vercel.json` (the daily 21:10 UTC backstop) → `src/lib/cron/tick.ts` `TICK_STEPS` → terminal: `curl -s -H "Authorization: Bearer $CRON_SECRET" "$APP/api/cron/tick" \| jq '.data.health'` | "Every five minutes one tick runs games, snapshot, evaluate, badges, in that order. Games go first because the Friday settlement and the weekly rollover are clock-driven and must never wait on a rate-limited RPC; badges go last because a confirmed chain write is the slowest step. The deployment is on Vercel's free plan, so the five-minute beat comes from a GitHub Actions pinger and Vercel's own daily run is the backstop that lands just after the Friday settle. Every step is timed and isolated, and this health block is what a pinger or a human reads." |
