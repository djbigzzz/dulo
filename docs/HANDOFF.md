...
## 6. Pitch Video script (≤ 3:00, replaces the 14 Sep 2-minute shot list)

The form's Pitch Video is "sell your project", and judges open it first. **Target 2:55.** Rewritten 16 Sep for the plain names, the new page routes (`/predictions`, `/competition`, `/quests`, `/copy`) and the shipped positioning line; updated the same day for the three games, the predictions-first landing and starter points. The video itself is reworked on Thursday; the timings and beat order below are fixed.

Record Thu 17 Sep 14:00 to 17:00 Irish (the US session is open) at 1280x800 in an incognito window with Phantom, plus one 390 px phone insert from Phantom's in-app browser. Unlisted YouTube; link it in the README and the form.
...
- **Never fake a shot.** When a precondition fails, take the plan-B beat in §6.3. Rehearse on the recording target at 10:00 the same morning and decide the plan-B beats then.
- **Never claim a number the app has not produced.** As of 16 Sep there are no users, no partners, no minted badge and no billing, so this script carries no traction number at all. If one is ever added it comes from `npm run -s stats` run the same day, bots and the founder's own wallets excluded.
- **Only shipped quests.** Show quests from §3.1 only; nothing that is not built appears on screen or in the voice-over.
- **`APP` is where you record.** Production once the first deploy is live (`https://<project>.vercel.app`, then `https://dulo.fun` when it resolves). Until then, a local production build: `npm run build && npm start`, with `npm run tick:local` in a second terminal. Never show a URL bar that implies a domain which does not resolve.
- `DEMO` = the demo wallet, signed in on camera. `HOLDER` = one of the curated "Try a real holder" addresses (never the founder's own wallet). `TARGET` = a copy target that is not `DEMO`. `FRESH` = a wallet that has never signed in this Season, used only for the welcome insert.

Budget (2:55 total): vision hook on the predictions-first landing 0:12 · problem 0:13 · founder 0:10 · check a wallet and its proof 0:40 · sign in, welcome and quests 0:25 · predict, then compete 0:25 · where it goes 0:30 · what we will not build, and the ask 0:20. The verified core (check, proof, sign in) is the longest block at 1:05.

### 6.1 Preconditions (check the morning of the shoot, in this order)

| # | Must be true where you record | How to check |
|---|---|---|
| P1 | The tick is healthy and running | `curl -s -H "Authorization: Bearer $CRON_SECRET" "$APP/api/cron/tick" \| jq '.data.health'` shows `"ok": true`; locally, `npm run tick:local -- --once` |
| P2 | `DEMO` holds $5 or more in each of 3 xStocks across 2 sectors, and First Position and Diversified are complete with a Proof button | `$APP/quests`, signed in as `DEMO`, On-chain filter |
| P3 | Diamond Hands is in progress at 3/7 or better (the tick has run daily since Tue) | the Diamond Hands card on `$APP/quests` |
| P4 | The competition week is open, bots are visible and labelled, and `DEMO` has exactly 2 paper trades in total (First Paper Trades counts every week), so the third lands on camera | `$APP/competition`; the First Paper Trades card on `$APP/quests` reads 2/3 |
| P5 | Three predictions are open (NVDA, TSLA, SPY) for this Friday, `DEMO` has not made a prediction yet (First Prediction reads 0/1) and its points balance is 100 or more (its starter points cover it) | `$APP/predictions`; the In-platform filter on `$APP/quests`; the account menu shows the balance |
| P6 | `TARGET` is not `DEMO`, has at least 2 legs, and no stale-price banner; every "Open in Jupiter" button opens the right xStock with the USDC amount filled in | `$APP/copy/<TARGET>`; open every leg once, off camera |
| P7 | `HOLDER` still holds at least 3 xStocks, and the page loads in under 3 seconds | `$APP/check/<HOLDER>` (open it once off camera; the answer is cached for 5 minutes) |
| P8 | The proof sheet shows the holding, the quantity after the multiplier, the dollar value, the price source and the snapshot time, and the address chip in the page header opens the wallet on Solscan | open a completed on-chain quest, then Proof. Correction to the 15 Sep list: there is no Solscan **token** link inside the proof sheet; the explorer link is the address chip |
| P9 | No placeholder partner, and every partner card reads "Quests live" or "Coming soon". Note what the per-quest completion counts actually read; if they are all zero, describe the mechanism, never a result | `$APP/partners` |
| P10 | The recording target is decided and stable: production if the deploy is live, otherwise a local production build with the tick running | open every URL in the shot list once, off camera |
| P11 | `FRESH` has never signed in on the recording target this Season, so its first sign-in shows the welcome toast | sign-in history on the target; never reuse `DEMO` for this |
| P12 | The landing's first screen at 1280x800 shows the headline, "Predict. Compete. Complete on-chain quests.", the welcome offer line, three live prediction cards and the three game tiles (Predictions, Competition, On-chain quests), each tile with a number rather than a dash | `$APP/` in the incognito window, signed out |

### 6.2 Shot list

| t | Beat | Shot | URL | Voice |
|---|---|---|---|---|
| 0:00 | Hook | Landing first screen, signed out, cursor still: the headline "The entertainment layer for xStocks.", then "Predict. Compete. Complete on-chain quests.", this week's live prediction cards beside it, the welcome offer line, and the three game tiles (Predictions, Competition, On-chain quests) underneath | `$APP/` | "Dulo is the entertainment layer for xStocks. Predict, compete and complete on-chain quests: three games, one Season leaderboard, on an engine built to score any tokenized asset, for any app or issuer." |
| 0:12 | Problem | Hold on the hero paragraph, then a slow scroll past the game tiles to the "Check any wallet" section. Lower third: "800,000+ Solana addresses hold a tokenized stock (Blockworks via Solana Compass, 12 Sep 2026)" | `$APP/` | "More than 800,000 Solana addresses hold a tokenized stock. Getting someone to buy once is the easy part. Giving them a reason to hold, to diversify and to come back is the hard part." |
...
| 0:35 | Check a wallet | Paste box on the landing, tap a "Try a real holder" chip (Public holder A), then the check page: the stat strip, the holdings list with quantities, the multiplier note and the price chip showing source and age | `$APP/` → `$APP/check/<HOLDER>` | "You need no account to see it work. Paste any Solana address. Dulo reads that wallet's xStocks straight from Token-2022 balances, applies the split and dividend multiplier, prices every position with the source and the age on the chip, and shows the on-chain quests it already meets. Nothing is stored, nothing is scored." |
| 0:55 | Proof | Proof button on a completed on-chain quest, then the proof sheet rows. Pause on the holding, the quantity, the dollar value, the price source and the snapshot time. Click the address chip in the header, land on Solscan, come back | `$APP/check/<HOLDER>` → `https://solscan.io/account/<HOLDER>` | "Every on-chain quest carries its evidence: the exact holding, the quantity after the multiplier, the value, the price source and the moment the engine read it. You do not take that on trust. Open the wallet on Solscan and check it yourself." |
| 1:15 | Sign in and quests | Two-second insert of `FRESH`: Connect → sign → the welcome toast (1,000 starter points for predictions, $10,000 of virtual cash for this week's competition). Then `DEMO`, already holding: Connect → Phantom → sign the message → the quests board. In-platform filter first (First Prediction and First Paper Trades waiting, with their next steps), then the On-chain filter: First Position and Diversified complete, Diamond Hands, Steady Buyer and Earnings Holder with their hints, and the two partner quests marked "Coming soon" | `$APP/` → `$APP/quests` | "Sign one message: no transaction. Every new player gets 1,000 starter points and ten thousand dollars of virtual cash, so nobody hits a dead end, and starter points never count toward rank. With my own wallet, First Position and Diversified verify in seconds. On-chain quests pay for holding, diversifying and buying steadily, never for trading volume. Points only, no cash value." |
| 1:40 | Predict · Compete | **Phone insert** (Phantom in-app browser, 390 px), predictions first: the Predict tab, Yes on NVDA with the default 100 points, the pool bar moves, "Quest complete: First Prediction". Cut to the Compete tab: the weekly competition (virtual cash), the third paper trade, the price chip shows source and age, "Quest complete: First Paper Trades". Cut to the copy page on desktop and open one "Open in Jupiter" leg | `$APP/predictions` → `$APP/competition` → `$APP/copy/<TARGET>` | "Then play, starting with predictions: Yes or No on Friday's close, for points, settled from the close with the source on the card. The weekly competition gives you ten thousand dollars of virtual cash at live prices, and the top ten with three trades earn points. In-platform quests complete as you go. Copying a portfolio opens one prefilled Jupiter swap per leg, signed in your own wallet." |
| 2:05 | Where it goes | Partner grid, then the xStocks partner page with its on-chain quests and per-quest completion counts, the "Coming soon" partner quests, then the "List your project" card | `$APP/partners` → `$APP/partners/xstocks` | "Every quest is a row of JSON, not code, so listing one is a database row. A partner page shows the on-chain quests a project listed and counts every verified completion it drives. That is the direction: an issuer's own points reward activity in its own venues, while Dulo scores behaviour across apps and issuers and the player keeps one score. The plan is that partners would pay per verified completion, apps and issuers would sponsor competitions, and players stay free. Nobody is billed today." |
| 2:35 | Ask | Back to the landing first screen, then the closing card. End card text: the positioning line · the live URL (only once it resolves) · `github.com/dulofun/dulo` · "Points only, no cash value." · "Not investment advice. xStocks are not available to U.S. persons or in restricted jurisdictions." · "Dulo is independent and not affiliated with xStocks (Backed Finance owns that brand)." | `$APP/` | "Dulo takes no custody, signs no swap for you, runs no real-money market and puts no points on chain. Hold a tokenized stock? Paste your wallet. Just want to play? Sign in and start with 1,000 starter points. Build on Solana? List a quest. Dulo. The entertainment layer for xStocks. Compete, predict and get rewarded, for points." |

### 6.3 Plan B (decide at the rehearsal; never fake a shot)

- **Not deployed by the shoot:** record from a local production build (`npm run build && npm start`, plus `npm run tick:local`). Keep the URL bar out of frame or let it read localhost. Do not say "live" of anything that is not, and do not put a live URL on the end card until one resolves.
- **P7 fails (the check page is slow or the holder sold):** try the next curated address. If none works, cut 0:35 to 0:55 and open on the signed-in quests board instead, giving the proof beat the extra 20 seconds and reading the proof rows slowly.
- **P8 fails (no price source in the proof sheet):** do not click through to Solscan. Say "the exact holding, the quantity after the multiplier and the value the engine used", and stop there.
- **P2 fails (First Position not complete):** do not record. Fix the read first (Refresh on `$APP/quests`, or run a tick), because this beat is the whole thesis.
- **P3 below 3/7:** show the Diamond Hands card without reading a number off it.
- **P11 fails (no fresh wallet, or no toast):** skip the insert. Show the account menu instead (Points balance, Season points, "Points only, no cash value") and say "every new player starts with 1,000 starter points", never "you just got". The welcome toast only appears on a wallet's first sign-in in a Season, so `DEMO` never shows it.
- **Sign-in does not refresh the board:** sign in off camera and start the 1:15 beat already connected. Say "I am connected with my own wallet" instead of "sign one message".
- **P4 fails:** if the third paper trade does not complete First Paper Trades on camera, do not mention the quest. If the week has no bots, cut the competition shot and give its seconds to predictions; the phone insert moves there.
- **P5 fails (no open markets, or `DEMO` already predicted):** with no open markets, cut the prediction shot and fold its sentence into the competition beat. If First Prediction is already complete, record the prediction anyway and do not mention the quest.
- **P12 fails (a tile shows a dash, or the prediction cards read "Between weeks"):** hold the hook on the headline and the three verbs only, and do not pan to the tiles or cards until they show numbers. Never describe a number the screen does not show.
- **P6 fails (a leg opens the wrong token, or no amount):** show the allocation and the plan card without clicking through, and drop the word "prefilled".
- **P9 fails (a partner card is wrong):** hold on the partner grid only, and take the "Where it goes" beat over the "List your project" card.
- **Settlement wording:** never say "Pyth" on camera. The card prints the real source, and "settled from the Friday close with the source shown on the card" is accurate.
...
| 2:10 | The honest limit: a second issuer | `prisma/schema.prisma` `Play.assetSource`, then the terminal: `git grep -n assetSource -- src prisma` (the seed writes it, nothing reads it), then the concrete import at the top of `src/lib/cron/snapshot.ts` and `src/lib/cron/evaluate.ts` | "Here is the one thing that is not done. Every Play row carries an assetSource column, but the snapshot and evaluate steps import the xStocks source directly, and a stored holding carries no source tag. So today a quest is scored against whatever the one catalogue returned. Before a second issuer can be scored, that needs an AssetSource registry keyed by Play.assetSource and a source tag on each holding, so each quest is evaluated against its own catalogue and mint set. The interface, the ids and the ledger already generalise; the binding does not." |
