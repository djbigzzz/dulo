...
> **Live since 21 Sep at https://dulo-iota.vercel.app:** still no external users (one account so far: the founder's), no partners, no minted Badge, no billing. Every shot in §6 and §6b is something the app does today, recorded from production once the first deploy is live and from a local production build until then. No script in this file may quote a number the app has not produced.
> **Plain names (updated 16 Sep):** public copy says predictions, the competition (always with "virtual cash" or "paper" next to it), quests, copy a portfolio, points, leaderboard, badges, partners. "Rewards" was the public noun for a Play from 15 to 16 Sep and is retired. Play, League, Call, Mirror, Scout and Oracle survive as internal code names only (Prisma models, `playKey` values, `/api/v1` paths), and this document uses them where it explains the code. Pages are `/predictions`, `/competition`, `/quests` and `/copy` (old paths redirect permanently), plus `/prestocks` (Pre-IPO, 22 Sep), `/check`, `/profile`, `/partners` and `/leaderboard`.
...
| Mechanics shipped | Decided 16 Sep: three games on one Season leaderboard. (1) Calls = predictions, points-only Yes/No on Friday closes, first in the nav. (2) The paper League = the weekly competition with $10,000 of virtual cash at real xStock prices and, since 22 Sep, PreStocks pre-IPO prices, top 10 score; it is not real trading and not swaps. (3) Plays = quests: in-platform quests anyone completes with virtual cash and points, and on-chain quests verified from the user's own wallet, with partner quests (Kamino, Jupiter Recurring) shown as coming soon. Mirror = copy a portfolio is a tool, not a fourth game (allocation view + Jupiter deep links, no in-app execution); its completion is the Portfolio Match quest. Soulbound Badges |
...
| Issuer scope | Season 0 reads xStocks. Every asset read goes through `AssetSource`, so another issuer is another implementation behind the same interface, not a rewrite. Since 22 Sep PreStocks pre-IPO tokens are that second implementation: read, priced, scored and paper-traded in the virtual-cash competition through the same interfaces. |
...
Updated 22 Sep: 23 rows (19 on 16 Sep, plus the two PreStocks on-chain quests and the two pre-IPO paper-trade quests). The `key` column is the DB key (`playKey`) and never changes; the title is what the app shows, from `src/lib/plays/catalogue.ts`. "Kind" is how `/quests` groups a row: in-platform (house partner, virtual cash and points), on-chain (an issuer partner, xStocks or PreStocks, verified from the user's wallet) or partner coming soon (seeded with `isActive=false`, never evaluated). Every on-chain row carries `Play.assetSource`, the issuer it is fenced to: `xstocks` for the nine xStocks rows, `prestocks` for Pre-IPO Position and Held Through a Split. The two in-platform pre-IPO rows carry `prestocks` too, for the card's issuer pill, hint and proof noun; the engine does not apply the issuer fence to internal events, and their `assetSymbols` list does the narrowing instead.
...
| first_preipo_trade | First Pre-IPO Trade | in-platform | `internal_event` event league_trade count 1, assetSymbols the eight pre-IPO symbols (`PRE_IPO_SYMBOLS`; assetSource `prestocks`) | 50 | |
| preipo_trio | Pre-IPO Trio | in-platform | `internal_event` event league_trade count 3 distinctBy symbol, assetSymbols the eight pre-IPO symbols (assetSource `prestocks`) | 100 | |
...
| pre_ipo_position | Pre-IPO Position | on-chain | `hold_any` minUsd 0 (any PreStocks pre-IPO token, counted by the token; assetSource `prestocks`) | 100 | |
| held_through_split | Held Through a Split | on-chain | `multiplier_change` (a pre-IPO token held across a ScaledUiAmount change: the same raw balance in consecutive day-end snapshots, a new multiplier between them; assetSource `prestocks`) | 150 | |
...
Notes:
- **Totals.** The 10 live in-platform quests are worth 1,000 points, the 11 live on-chain quests 2,950, and the 2 coming-soon partner quests 500. The in-platform total is 850 from the eight 16 Sep rows plus 150 from the two pre-IPO paper-trade rows (50 for First Pre-IPO Trade, 100 for Pre-IPO Trio). The on-chain total is 2,700 from the nine xStocks quests plus 250 from the two PreStocks quests (100 for Pre-IPO Position, 150 for Held Through a Split). One ladder covers every quest: tier 1 (first minute) 50 to 100, tier 2 150 to 250, tier 3 300 to 500. Weekly competition finishes (100 to 1,000) sit above any single quest.
- **distinctBy** is an optional `internal_event` field (`src/lib/plays/rules.ts`): `ref` counts one event per question (both sides of one question count once, top-ups add nothing), `symbol` one per upper-cased symbol (an xStock or a pre-IPO token; the progress unit reads "xStocks" under the Season 0 issuer and "assets", rendered as pre-IPO tokens, under `prestocks`), `day` one per UTC calendar day.
- **assetSymbols on an internal_event rule (22 Sep).** The scope field every rule already accepts (`src/lib/plays/rules.ts`) narrows a counted event to those whose `meta.symbol` (trimmed, upper-cased) is in the list; an event without a symbol is skipped, and the proof echoes the list as `symbols`. A rule without it, or with an empty list, evaluates byte-for-byte as before (`tests/internal-event-symbols.test.ts`). No new rule key, no schema change, no `EvalContext` change; the keys `symbols` and `source` still fail parsing.
...
- **Pre-IPO Position (22 Sep).** The first quest of the second issuer, PreStocks, under the listed `prestocks` Partner (campaign "Pre-IPO on Dulo · Stocks Season"). It is count-based on purpose: every PreStocks pre-IPO token is priced by Jupiter only (no Pyth feed), and `lib/price` caches are empty on a cold serverless start, so a USD floor could read incomplete for a wallet that plainly holds the token. `hold_any` with `minUsd: 0` completes on any in-scope position with qty > 0, priced or not. The fence does the rest: the engine drops every holding whose `source` is not `prestocks`, so an xStock can never complete it, and no xStocks quest can be completed by a pre-IPO token (`tests/second-issuer-regression.test.ts`). The description is a wallet state ("Your connected wallet holds any PreStocks pre-IPO token..."), never an instruction to buy, and it says pre-IPO token, never share, equity or stock.
- **Held Through a Split (22 Sep).** The second PreStocks quest, `held_through_split`, is the only row on the `multiplier_change` rule type, fenced to `prestocks` like Pre-IPO Position. The engine (`evalMultiplierChange`) walks day-end snapshots and completes when an in-scope asset shows the same raw balance under a new Token-2022 ScaledUiAmount multiplier at the day-end of the NEXT calendar day, and only when the mint's own record corroborates it: `buildEvalContext` passes `listCorporateActions()` as `ctx.corporateActions`, and the pair scores only if the earlier snapshot shows the record's `multiplierBefore`, the later its `multiplierAfter`, and the record's `effectiveAt` falls between the two ticks. A snapshot pair alone never scores (a degraded mint read in one tick cannot look like an adjustment, and the adapter now fails a wallet read outright rather than assuming multiplier 1 for a mint it could not read), a missed cron day is not a pair, and a day-end without the asset or with a different raw balance (sold out, trimmed or added to) is "not held through", so selling out and buying back never counts. The proof carries the before and after observations (day, multiplier, qty, raw) and the ratio. The copy is a wallet state, never an instruction, and it never presents the adjustment as a price signal: an adjustment changes the number of tokens shown, not the holder's value. Both PreStocks adjustments on record (SpaceX 1 -> 5, effective 10 Jun 2026; OpenAI 1 -> 1.4861347, effective 17 Jul 2026; verified on-chain 22 Sep with `getScaledUiAmountConfig`) predate Dulo's first production snapshot of 21 Sep 2026, so no wallet on record held through either and the quest cannot have completed yet; the description says so ("Completes on the next adjustment"). 150 points, no badge.
- **First Pre-IPO Trade and Pre-IPO Trio (22 Sep).** The two in-platform pre-IPO quests: `first_preipo_trade` (50 points, one `league_trade` event) and `preipo_trio` (100 points, three `league_trade` events distinct by symbol), both under the house partner ("Dulo games · Stocks Season"), both with `assetSymbols: PRE_IPO_SYMBOLS` (ANDURIL, ANTHROPIC, FIGUREAI, KALSHI, NEURALINK, OPENAI, POLYMARKET, SPACEX, pinned equal to `PRESTOCKS_STATIC` by test) and `assetSource: prestocks`. `league/trade` evaluates them in the same request as every other in-platform quest, so the toast lands with the fill. Their copy is an in-app action, never a buy of a real token: "Place your first paper trade in a pre-IPO token with virtual cash. Points only, never real money." and "Place paper trades in three different pre-IPO tokens with virtual cash. Trades from any week count, and sells count too." Two wording decisions: "no cash value" is not in a quest description because `tests/quest-compliance.test.ts` bans the phrase "cash value" there, and Pre-IPO Trio counts trades from any week because `internal_event` has no week window (exactly like Paper Portfolio), so the copy promises no weekly window. No badge.

Rule engine types: hold_any, hold_consecutive, net_increase_days, hold_through_date, diversified, mirror_match, internal_event, multiplier_change. Pure functions over normalised Holding[] history (and internal events). Tests per rule type with fixture snapshots. multiplier_change (22 Sep) completes when an in-scope asset was held across a Token-2022 ScaledUiAmount change (the same raw balance, a new multiplier); its one catalogue row is `held_through_split` (Held Through a Split, PreStocks). The corporate actions an issuer Partner page shows come from `src/lib/corporate-actions.ts` (`listCorporateActions`: one batched mint read over every registered issuer, cached 10 minutes, never throws), which reads the same ScaledUiAmount state the engine does; a split or an adjustment changes the number of tokens shown, never the holder's value.

### 3.2 League (the weekly competition, virtual cash)
Weekly, Mon 00:00 UTC to Fri 20:00 UTC. Virtual $10,000 per user per week (`VIRTUAL_CASH_USD === STARTING_CASH_USD`, tested). Trade any xStock, or since 22 Sep any PreStocks pre-IPO token, at lib/price with 0.1% virtual spread, $10 minimum. Equity recomputed on the 5-minute tick, plus on read when the last recompute is more than 60s old (recomputeIfStale). Top 10 get 1000/700/500/300/200/100×5 points. Seed 15 bot accounts with 3–8 plausible trades.
...
Decided 22 Sep (founder): pre-IPO tokens are tradable in the competition, always with virtual cash.
- The same $10,000 weekly account, 24/7, and a pre-IPO position counts on the weekly board like any xStock. `LEAGUE_ASSET_SOURCES = ["xstocks", "prestocks"]` (`src/lib/games/league.ts`) replaces the old single `LEAGUE_ASSET_SOURCE`; `placeTrade` quotes with `getPriceBySymbol(symbol, { sources: LEAGUE_ASSET_SOURCES })` (`PriceSymbolOptions.sources` in `src/lib/price.ts` is the union fence, and the old `{ source }` still works), and a symbol no registered source knows answers 400 `Unknown xStock or pre-IPO token: <symbol>` with nothing written. `recomputeEquity` prices a pre-IPO position through `getPrices` beside the xStocks and ranks it with them; an unpriced pre-IPO position is valued at cost and reported, never dropped. The Friday rollover pays a pre-IPO leader like any other (`tests/league-preipo.test.ts`).
- House bots stay on xStocks: `BOT_ASSET_SOURCE = "xstocks"`, `TRADABLE_SYMBOLS` is the xStocks list only, and `botQuotes()` fences its price call to that source and reads back only `TRADABLE_SYMBOLS`, so no `planBotTrades` plan and no `seedBots` row is ever a pre-IPO token.
- The symbols endpoint (`GET /api/v1/league/symbols`, `getLeagueSymbols` in `src/lib/games/league-views.ts`) lists `TRADABLE_SYMBOLS` in order, then every pre-IPO symbol from `prestocks.listAssets()`, then anything else the caller holds, each tagged `source` (`"xstocks"` or `"prestocks"`; absent means xstocks). A pre-IPO entry also carries `issuerMark` (`{ price, publishedAt }` from `getPreStocksMarks()`, null when the issuer gave no positive mark) and `change24h` (Jupiter's `priceChange24h`, null when absent). Marks and 24h changes are cached per instance for `PRE_IPO_EXTRAS_TTL_MS` (60 s) and never throw; `resetLeagueSymbolsCache()` is the test hook. The overview `LeagueResponse.quotes` list did not change. The issuer's mark and the pool quote are two numbers shown as two things, never a discount, a premium or a gap.
- The Pre-IPO page is `/prestocks` (desktop nav item "Pre-IPO", between Quests and Copy a portfolio; the mobile tab bar stays five tabs), and `PRE_IPO_COMPLIANCE_LINE` renders beside `COMPLIANCE_LINE` wherever a pre-IPO token is on screen. Pools are thin: a virtual-cash fill there is a quote a real trade of that size would move, and the copy never presents it as a price a real trade would get. Points only, so nobody rational moves a market for them.
...
- PreStocks (second issuer, 22 Sep): its tokens are "pre-IPO tokens" or "tokenized pre-IPO exposure", never shares, equity or stock (never "SpaceX stock"). The issuer's mark and the pool price are two different numbers, shown as two things and never as a discount. Its AssetSource name is `prestocks` (`src/lib/assets/prestocks.ts`, registered in `src/lib/assets/registry.ts`); the Partner slug is `prestocks` and the quest keys are `pre_ipo_position` and `held_through_split`. Since 22 Sep (founder decision) pre-IPO tokens are tradable in the competition, always with "virtual cash" beside the word: the same $10,000 weekly account, 24/7, on the weekly board, while house bots trade xStocks only. The desktop nav item is "Pre-IPO" at `/prestocks`, between Quests and Copy a portfolio; the mobile tab bar stays five tabs. The landing carries one small pre-IPO hook line beside the market-session chip and nothing else: the headline, the three verbs, the three game tiles, the welcome line and the positioning line are byte-identical, and the copy tool stays xStocks-only. The in-platform quest titles are "First Pre-IPO Trade" and "Pre-IPO Trio"; each describes an in-app paper trade and never tells anyone to buy a real token. `PRE_IPO_COMPLIANCE_LINE` renders beside `COMPLIANCE_LINE` wherever a pre-IPO token is on screen.
- Corporate actions (22 Sep): a Token-2022 ScaledUiAmount change is "an on-chain adjustment", or "a split" when the ratio is a whole number of 2 or more (SpaceX's 5-for-1). It is never a price signal, a gain or a discount: an adjustment changes the number of tokens shown, not the holder's value, and every line of copy about one says so. "Held Through a Split" is the quest title (key `held_through_split`); its description is a wallet state ("Your wallet held a pre-IPO token across an on-chain adjustment...") and never an instruction.
...
The form's Pitch Video is "sell your project", and judges open it first. **Target 2:55.** The voice-over below is **389 words** (374 on 20 Sep, plus one 15-word sentence on 22 Sep for the pre-IPO cut in the Predict · Compete beat), which is 2:47 at 140 words a minute and just under 3:00 at a slow 130 — so it fits at a natural pace, with less room to breathe than before. The 16 Sep version ran 451 words, about 3:07, and could only be delivered rushed. Do not add sentences back without cutting others; if the rehearsal runs long, the pre-IPO sentence is the first cut (§6.3, P10).
...
- **Pre-IPO on camera (22 Sep):** say "pre-IPO token", never share, stock or equity; the issuer's mark and the pool price are two numbers, never a discount; and "virtual cash" sits beside every pre-IPO trade.
...
- **Never claim a number the app has not produced.** There are no external users (one account so far: the founder's), no partners, no minted badge and no billing, so this script carries no traction number at all. If one is ever added it comes from `npm run -s stats` run the same day, bots and the founder's own wallets excluded.
- **Only shipped quests.** Show quests from §3.1 only; nothing that is not built appears on screen or in the voice-over.
- **`APP` is https://dulo-iota.vercel.app**, live since 21 Sep. Record there: the main track is judged partly on a working end-to-end demo, and a visible live URL is the cheapest proof of one. dulo.fun is not registered, so nothing on screen or on the end card may show it. Only if production is down on the day, fall back to a local production build (§6.3).
...
| P10 | `$APP/prestocks` lists the eight pre-IPO tokens, each with a price chip showing source and age and the issuer's mark as a separate number, and the trade form accepts a $10 paper trade in the one you will trade | `$APP/prestocks` off camera; if a token has no Jupiter quote, pick another |
...
| 1:42 | Predict · Compete | Predictions: Yes on NVDA with the default 100 points, the pool bar moves, "Quest complete: First Prediction". Cut to the competition: one paper trade, the price chip showing source and age. Then the Pre-IPO page: one paper trade in a pre-IPO token with virtual cash, the issuer's mark and the pool price as two numbers, "Quest complete: First Pre-IPO Trade". Cut to the copy page and open one "Open in Jupiter" leg | `$APP/predictions` → `$APP/competition` → `$APP/prestocks` → `$APP/copy/<TARGET>` | "Then play. Predictions: Yes or No on Friday's close, for points, settled from the close with the source on the card. The weekly competition gives you ten thousand in virtual cash at live prices, and the top ten with three trades earn points. Pre-IPO tokens from PreStocks trade in the same competition, with virtual cash, around the clock. Copying a portfolio opens one prefilled Jupiter swap per leg, signed in your own wallet." |
...
- **P10 fails (no pre-IPO quote, or the paper trade is refused):** cut the `$APP/prestocks` shot and its one sentence; the beat still fits.
...
8. **Submit.** `docs/SUBMISSION.md` has the paste-ready copy. Press **Submit Project** — Save Draft is not a submission — then keep editing until the close on Fri 25 Sep 2026, 16:00 ET.

## 6b. Technical Video script (≤ 5:00)

The form's Technical Video is a walkthrough of the code. **Target 4:55.** The voice-over is 700 words (about 4:30 at 150 words a minute); the 22 Sep draft ran 925 and could not fit. Rewritten 16 Sep: plain names on screen (predictions, the competition with virtual cash, quests, copy a portfolio), code names in the code, the starter grant and the Season points rule in the ledger beat, and the one honest limit named out loud.

Record from the editor (VS Code, 16 pt, minimap off), a terminal, and a browser on https://dulo-iota.vercel.app. Unlisted YouTube; link it in the README and the form. Code names survive in the code, so `Call`, `League`, `Play` and `Mirror` will be on screen: say once, early, that the user-facing names are predictions, the competition, quests and copy a portfolio, and then read the code as it is. The page routes on screen are the new ones (`/predictions`, `/competition`, `/quests`, `/prestocks`, `/copy`); the `/api/v1` paths keep their code names.
...
- A tick ran in the last 5 minutes with `health.ok: true`: the Actions page shows a green "Cron tick (Hobby fallback)" run, or `curl -s -H "Authorization: Bearer $CRON_SECRET" "$APP/api/cron/tick" | jq '.data.health'`.
...
- No Badge has been minted on mainnet as of 22 Sep (no funded server wallet). Unless one exists by the shoot, take the plan-B beat at 4:25 rather than a Solscan transaction.

| t | Beat | Screen | Voice (key points) |
|---|---|---|---|
| 0:00 | What it is | Two seconds of the landing first screen (the live prediction cards and the three game tiles), then the README top, then the `src/lib` tree | "Dulo is a Next.js 15 app reading Solana mainnet: predictions, a virtual-cash competition and quests on one Season leaderboard. There is no Anchor program: every read is public chain state, and the only transaction this codebase sends is a soulbound badge mint. The screens say predictions, competition, quests; the code still says Call, League, Play and Mirror, and only the code does." |
| 0:25 | The four interfaces | `src/lib/core/types.ts`: `ChainAdapter`, `AssetSource`, `PriceSource`, `GameModule`; then `src/lib/core/caip.ts` | "Everything sits behind four interfaces: chain reads through ChainAdapter, asset maths through AssetSource, prices through lib/price, and the two clock-driven games as GameModules. Asset ids are CAIP-19 strings, so an asset carries its chain in its id, and a user owns many wallets: nothing keys on one pubkey. That is what makes a second asset class an implementation, not a rewrite." |
| 1:05 | Token-2022 multipliers | `src/lib/adapters/solana.ts`: `getTokenBalances` over both token programs (`getParsedTokenAccountsByOwner`), the batched `getMultipleParsedAccounts` mint read, `parseScaledUiAmountExtension`, `effectiveMultiplier`; then `normaliseQtyString` in `src/lib/assets/normalise.ts`; then `tests/solana-adapter.test.ts`; then (15 s) the corporate-actions feed: `src/lib/corporate-actions.ts` `listCorporateActions` → `GET /api/v1/partners/prestocks` `corporateActions` in the terminal (SpaceX 1 → 5, kind split; OpenAI 1 → 1.4861347, kind adjustment) → the PreStocks Partner page in the browser → `src/lib/plays/catalogue.ts` the `held_through_split` row | "xStocks and PreStocks are Token-2022 mints with the ScaledUiAmount extension: a split or a dividend changes a multiplier on the mint, not your raw balance. The adapter batches the mint reads, switches to the pending multiplier once its activation time passes, and normalises quantity with string decimal maths. The same read feeds the corporate-actions feed, cached ten minutes and never throwing. Two adjustments are on record, SpaceX five-for-one and OpenAI times one point four eight six, and the copy says what that means: more tokens shown, the same value held." |
| 1:35 | A quest is a data row | `src/lib/plays/catalogue.ts`, the Diversified rule `{ "type": "diversified", "minAssets": 3, "minSectors": 2 }` and one in-platform rule with `distinctBy` → `src/lib/plays/rules.ts` `PLAY_RULE_TYPES` (eight types, zod-validated) → `src/lib/plays/engine.ts` `evaluatePlay` → `evalDiversified`; then the browser, Proof on a completed on-chain quest; then (20 s) the pre-IPO trade path: `src/lib/plays/catalogue.ts` the `first_preipo_trade` row (`assetSymbols` narrows counted paper trades to the eight pre-IPO symbols) → `src/lib/games/league.ts` `LEAGUE_ASSET_SOURCES` and `placeTrade`'s `getPriceBySymbol(symbol, { sources })` → the browser: `$APP/prestocks`, one paper trade in a pre-IPO token with virtual cash, the toast "Quest complete: First Pre-IPO Trade" | "A quest is data: a JSON rule on a row, validated by a zod schema with eight rule types, evaluated by pure functions over history: complete or not, plus the proof the player opens in the drawer. Adding a quest is a database row, not code. In-platform quests use the same engine over paper trades and predictions, and the competition takes one allowlist of asset sources, so a pre-IPO paper trade quotes through the same price layer as an xStock. Bots stay on xStocks." |
| 2:10 | A second issuer through the same interfaces | `src/lib/assets/registry.ts` `ASSET_SOURCES` (two entries: xstocks, prestocks) → `src/lib/assets/prestocks.ts` `PRESTOCKS_STATIC` (eight mints, `pythFeedId: null`) → `prisma/schema.prisma` `Play.assetSource` → `src/lib/plays/engine.ts` `scopeOf` and `inScope` → `src/lib/plays/catalogue.ts` the `pre_ipo_position` row (`assetSource: "prestocks"`) → the terminal: `npx vitest run tests/second-issuer-regression.test.ts tests/pre-ipo-quest.test.ts`, run live | "Here is the second issuer. PreStocks enters through the AssetSource registry: the snapshot reads the union of every registered mint set in the batched RPC calls it already made, so the second issuer costs zero extra calls; each holding is tagged with the source that claimed it; and a pre-IPO token is Jupiter-priced and never a Hermes call. Then the fence: every Play row carries an assetSource, every holding carries its source, and the engine applies both, the issuer fence and then the rule's own asset list. The regression file runs every xStocks quest with and without a bag of pre-IPO tokens and proves the answer never moves. One scoring path, two issuers, keyed by Play.assetSource." |
| 2:50 | The five-minute tick | `.github/workflows/tick.yml` (`*/5`) and `vercel.json` (the daily 21:10 UTC backstop) → `src/lib/cron/tick.ts` `TICK_STEPS` → terminal: `curl -s -H "Authorization: Bearer $CRON_SECRET" "$APP/api/cron/tick" \| jq '.data.health'` | "Every five minutes one tick runs games, snapshot, evaluate, badges, in that order: games first because the Friday settlement and the weekly rollover are clock-driven and must never wait on a rate-limited RPC, badges last because a confirmed chain write is the slowest step. On Vercel's free plan the five-minute beat comes from a GitHub Actions pinger, with Vercel's daily run as the backstop after the Friday settle. Every step is timed and isolated; this health block is what a human reads." |
| 3:25 | Append-only ledger | `prisma/schema.prisma` `PointsEvent` with `@@unique([userId, seasonId, ref])` → `src/lib/games/ledger-policy.ts` (`STARTER_POINTS`, `NON_SCORING_SOURCES`, `starterRef`) → `src/lib/games/starter.ts` `ensureStarterPoints` (`createMany` with `skipDuplicates`, granted only when count is 1) → `src/lib/cron/evaluate.ts` (PointsEvent written before PlayProgress, a P2002 means already awarded) → `src/lib/games/calls.ts` with its negative `delta` → `REAL_USER_WHERE` and `seasonScoreWhere` in `src/lib/server/queries.ts` | "Points are an append-only ledger and a balance is a sum. Every row carries a unique ref, like play colon first underscore position, so a retry, a crash or two sign-ins at once can never pay twice. Points put into a prediction are a negative event. Season points are the same sum without starter rows, house seed rows and open predictions, and one query fragment feeds every board. Bots keep both boards busy; they are filtered in the database, never score and cannot sign in." |
| 4:00 | Copying a portfolio is links, not execution | `src/lib/mirror/allocation.ts` `jupiterSwapUrl` and `src/lib/mirror/events.ts` (the recorded intent the rule consumes) → terminal: `git grep -n sendAndConfirmTransaction -- src` (one file, `src/lib/badges/mint.ts`) | "Copying a portfolio builds an allocation plan and one Jupiter link per leg: sell USDC, buy the xStock, amount prefilled. You sign in Jupiter, in your own wallet; pressing verify records an intent the next snapshot is compared against. The only transaction this codebase ever sends is the badge mint, and that grep is the proof." |
| 4:25 | Badges, tests, CI | `src/lib/badges/mint.ts` instruction list, then the terminal: `npx vitest run` summary line, then `.github/workflows/ci.yml` and the green Actions run | "Badges are Token-2022 mints with NonTransferable and MetadataPointer: one transaction, zero decimals, supply of one, mint authority removed in the same transaction, no custom program. [Read the number the terminal prints] tests cover every rule type, the adapter, the ledger, the starter grant and both games, and CI runs lint, typecheck, tests and a build on every push. MIT. Points only, no cash value." |

**Plan B:**

- **No mainnet Badge yet (true as of 22 Sep):** at 4:25 show `tests/badges.test.ts` asserting the instruction set against a faked send, say "the first mainnet mint is queued", and skip the explorer. Once a mint exists, open `https://solscan.io/tx/<txSig>` and the token page showing the Non-Transferable and Metadata Pointer extensions.
...
- **Live URL:** https://dulo-iota.vercel.app (dulo.fun is not registered). It must work in incognito with Phantom on desktop and phone: a fresh wallet must see the welcome toast and a points balance of 1,000. Note the production alias is public while the preview and branch URLs sit behind Vercel Authentication, which is the correct arrangement.
- **Videos:** Pitch Video ≤ 3:00 (§6) and Technical Video ≤ 5:00 (§6b), unlisted YouTube or a public Loom link, both linked in the README and the form. The technical walkthrough is on Loom since 22 Sep (4:59); the pitch still needs a take under 3:00.
- **Form fields:** Short Description ≤ 280 characters; Full Description Markdown ≤ 5,000; team Solo; bounty tracks none.
