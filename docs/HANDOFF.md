...
Updated 22 Sep: 20 rows (19 on 16 Sep, plus the one PreStocks quest). The `key` column is the DB key (`playKey`) and never changes; the title is what the app shows, from `src/lib/plays/catalogue.ts`. "Kind" is how `/quests` groups a row: in-platform (house partner, virtual cash and points), on-chain (an issuer partner, xStocks or PreStocks, verified from the user's wallet) or partner coming soon (seeded with `isActive=false`, never evaluated). Every on-chain row carries `Play.assetSource`, the issuer it is fenced to: `xstocks` for the nine xStocks rows, `prestocks` for Pre-IPO Position.
...
Notes:
- **Totals.** The 8 live in-platform quests are worth 850 points, the 10 live on-chain quests 2,800, and the 2 coming-soon partner quests 500. The on-chain total is 2,700 from the nine xStocks quests plus 100 from Pre-IPO Position. One ladder covers every quest: tier 1 (first minute) 50 to 100, tier 2 150 to 250, tier 3 300 to 500. Weekly competition finishes (100 to 1,000) sit above any single quest.
